...
\section{Registers, Main Widths, etc.}
	...
	\item ILARs
		...
		\item The two ILARs called "\texttt{ipc}" are the program counters
			...
			%5 bits
			%%(5 because 128 bytes long ILARs, and forced alignment of
			%%instructions)
			%of their addresses. The number of bits is (7 - 2 = 5)
			%because of alignment of
	...
	\item DLARs
		...
		\item The base address field of a DLAR is
			%(32 - 6 = 26)-bit.
			(64 - 6 = 58)-bit.
		\item Note that the full address of a scalar located in a DLAR,
			\texttt{dA}, is determined by the following formula:
			\texttt{cat(dA.base\_address, dA.scalar\_offset)}.
			If \texttt{dA.type} is a size other than that of
			\texttt{u8} and \texttt{s8},
			\texttt{dA.scalar\_offset} will be aligned to the size of
			\texttt{dA.type} for this calculation.
	...
	\item The \texttt{ie} register
		...
		\item The \texttt{retiz} and \texttt{retinz} instructions
			set \texttt{ie} to
...
\section{Vector Type Casting}
	\begin{itemize}
	%--------
	\item The scalar offset field determines which sub-vectors of a DLAR to
		use for vector operations when DLAR types do not match,
		%with the following restriction:
		%the type of operand \texttt{dB} determines the
		with the following behavior:
		\begin{enumerate}
		%--------
		% 1
		% to=d2: type=u16, soffs=7'b000_0000
		% from=d3: type=u8, soffs=7'b000_1000
		% up-cast:
		\item When up-casting any source DLAR, \texttt{dsrc},
			to the type of any destination DLAR, \texttt{ddst},
			the offset into \texttt{dsrc.data}
				of element \texttt{0x0} of the sub-vector
			of \texttt{dsrc.data} that will be used will
			be calculated by aligning \texttt{dsrc.scalar\_offset}
			%to the base 2 logarithm of the number of vector
			%elements in
			%\texttt{ddst}.
			%%\texttt{dsrc}.
			to \texttt{ddst.type}.
			%to \texttt{dsrc.type}.
		% 2
		\item When down-casting any source DLAR, \texttt{dsrc},
			to the type of any destination DLAR, \texttt{ddst},
			the offset into \texttt{ddst.data}
				of element \texttt{0x0} of the sub-vector
			of \texttt{ddst.data} that will be used will
			be calculated by aligning \texttt{ddst.scalar\_offset}
			%to the base 2 logarithm of the number of vector
			%elements in
			%\texttt{dsrc}.
			%\texttt{ddst}.
		...
		%\item When down-casting any source DLAR, \texttt{dsrc},
		%	to the type of any destination DLAR, \texttt{ddst},
		%	the index of element \texttt{0x0} of the sub-vector of
		%	\texttt{ddst.data} that will be used will
		%	be calculated by aligning \texttt{ddst.scalar\_offset}
		%	to the base 2 logarithm
		% 3
		%\item If \texttt{dB.type} and \texttt{dC.type} are
		%	both the same size, the following behavior holds:
		%	(1) if \texttt{dB.type} and \texttt{dC.type} are to be
		%	up-casted to the type of a
		%	destination DLAR, \texttt{ddst}, the sub-vectors
		%	of \texttt{dB.data} and \texttt{dC.data}
		%	that will be used are selected as in item \textbf{1.}
		%	in this list;
		%	(2) otherwise, if down-casting \texttt{dB} and
		%	\texttt{dC}, the sub-vector of a destination DLAR,
		%	\texttt{ddst}, that will be used for will be as in
		%	item \textbf{2.} of this list.
		\item If either of the following conditions is met:
			\begin{itemize}
			%--------
			%\item \texttt{dB.type} and \texttt{dC.type} are both
			%	smaller in size than the type of a
			%	destination DLAR, \texttt{ddst},
			%	(though \texttt{dB.type} and \texttt{dC.type}
			%	\textit{don't} have to be
			%	the same size)
			\item \texttt{dB.type} and \texttt{dC.type} are both
				smaller in size than the type of a
				destination DLAR, \texttt{ddst},
				and \texttt{dB.type} and \texttt{dC.type}
				are the same size
			\item \texttt{dB.type} and \texttt{dC.type} are both
				smaller in size than the type of a
				destination DLAR, \texttt{ddst},
			...
			%--------
			\end{itemize}
			then up-casting will be performed on both
			\texttt{dB} and \texttt{dC} as in
			item \textbf{1.} of this list.
		% 4
		\item If the following conditions are all met:
			\begin{itemize}
			%--------
			\item \texttt{dB.type} and \texttt{dC.type} are both
				the same size
			\item the shared size of
				\texttt{dB.type} and \texttt{dC.type} is larger
				than the size of the type of a
				destination DLAR, \texttt{ddst}
			%--------
			\end{itemize}
			then the down-casting will be performed on both
			\texttt{dB} and \texttt{dC} as in
			item \textbf{2.} of this list.
		% 5
		\item If the following conditions are all met:
			...
			%--------
			\item exactly one of
				\texttt{dB.type} and \texttt{dC.type}
				is smaller than the type of a
				destination DLAR, \texttt{ddst}
			\item there also exists a source DLAR,
				\texttt{dB} or \texttt{dC},
				whose type is the same size as \texttt{ddst.type}
			%--------
			\end{itemize}
			then only the source DLAR whose type is smaller than
			\texttt{ddst.type} will be up-casted, as in
			item \textbf{1.} of this list.
			The other source DLAR will be used as is, without
			casting.
		% 6
		\item If the following conditions are all met:
			\begin{itemize}
			...
			%--------
			\end{itemize}
			only the source DLAR with a type larger than
			\texttt{ddst.type} will be down-casted, as in
			item \textbf{2.} of this list.

			Also, the sub-vector of the source DLAR whose type is
			the same size as \texttt{ddst.type} will be selected as
			if item \textbf{2.} of this list were applied with the
			two source DLARs, where the "destination" DLAR for that
			operation is the source DLAR whose type is same size as
			\texttt{ddst.type}.

			Note that the source DLAR whose type is the same size
			as \texttt{ddst.type} will not have any of its fields
			changed by this second application of
			item \textbf{2.} of this list.
		%% 7
		%\item If the following conditions are all met:
		%	\begin{itemize}
		%	%--------
		%	\item \texttt{dB.type} and \texttt{dC.type} do not have
		%		the same size
		%	\item \texttt{dB.type} and \texttt{dC.type}
		%		are both larger in size than the type of a
		%		destination DLAR, \texttt{ddst}
		%	%--------
		%	\end{itemize}
		%	%then the source DLAR with the larger type will be used
		%	%then \texttt{dB.data} and \texttt{dC.data} will both be
		%	%temporarily casted to \texttt{ddst.type}, and
		% 7
		% TODO: continue here
		\item If the following conditions are all met:
			\begin{itemize}
			...
			\item \texttt{dB.type} and \texttt{dC.type} do not have
				the same size
			\item neither \texttt{dB.type} nor \texttt{dC.type}
				is the same size as a
				destination DLAR, \texttt{ddst}
			\item Either of the following conditions is met:
				\begin{itemize}
				%--------
				\item exactly one of
					\texttt{dB.type} and \texttt{dC.type} is
					has a larger size than \texttt{ddst.type}, and
					the type of the other source DLAR has a smaller
					size than \texttt{ddst.type}
				\item both of
					\texttt{dB.type} and \texttt{dC.type} is
					have larger sizes than \texttt{ddst.type}
				%--------
				\end{itemize}
			%--------
			\end{itemize}
			then \texttt{dC.type} will be instead interpreted as
			\texttt{dB.type}, and the specific item in this list
			which applies to that situation will be performed.
		%--------
		\end{enumerate}
	%--------
	\end{itemize}
\section{Exceptions}
	...
	\item When \texttt{xct == 0x3}:
		\texttt{retiz} or \texttt{retinz} when in user mode.
	\item When \texttt{xct == 0x4}:
		\texttt{retxz} or \texttt{retxnz} when in user mode.
...
\section{Instructions}
	\subsection{Group 0 Instructions}
		...
		\item Most instructions in this group use Encoding 0.
			...
			bit does for other instructions in Encoding 0.
		\item Instruction List:
			...
			\item \texttt{add dA, iB.addr, dC}
				...
				\item For \texttt{add.s dA, iB.addr, dC}, this instruction
					uses the value of
					%\texttt{cast(dA.type, align(u64, iB.addr))}
					\texttt{cast(dA.type, iB.addr)}
					as the second argument of the \texttt{add}.
	...
	\subsection{Group 2 Instructions}
		...
		%	data field (temporarily casted to the \texttt{u64} type) and
		...
		\item For these instructions, the value of
			\texttt{cast(u64, dB.scalar\_data) + cast(s64, simm12)}
	...
	\subsection{Group 3 Instructions}
		...
		\item Encoding: \texttt{0011 aaaa aabb bbbb  iiii iiii iiii oooo}
			...
			\end{itemize}
		\item For these instructions, the value of
			\texttt{cast(u64, align(dB.type, dB.addr)) + cast(s64, simm12)}
			is used as the address to load from.
		\item Also, the type \texttt{dA} is set to is indicated in the
			instruction name, with, for example, \texttt{ldu8} setting
			\texttt{dA}'s type to \texttt{u8}.
	...
	\subsection{Group 4 Instructions}
		...
		\item Encoding: \texttt{0100 aaaa aabb bbbb  cccc cc00 0000 oooo}
			...
			\end{itemize}
		\item For these instructions, the value of
			\texttt{cast(u64, dB.scalar\_data)
				+ cast(u64, dC.scalar\_data)}
			is used as the address to load from.
		\item Also, the type \texttt{dA} is set to is indicated in the
			instruction name, with, for example, \texttt{ldu8} setting
			\texttt{dA}'s type to \texttt{u8}.
	...
	\subsection{Group 5 Instructions}
		...
		\item Instruction List:
			...
			%\item \texttt{stf32i dA, dB, simm12}
			%\item \texttt{stf64i dA, dB, simm12}
			\item \textit{Reserved for future expansion.}
			\item \textit{Reserved for future expansion.}
			\item \textit{Reserved for future expansion.}
			\item \textit{Reserved for future expansion.}
	...
	\subsection{Group 7 Instructions}
		...
		%	\texttt{u64} type) of the \texttt{dC} DLAR.
		\item For instructions using Encoding 0, the address to
			\texttt{fetch} from is computed with the following formula:
			\texttt{cast(u64, align(u64, dC.scalar\_data))}.
		...
		%	\texttt{cast(s64, (isimm11 << 2))}.
		\item For instructions using Encoding 1, the address to
			\texttt{fetchi} from is computed with the following formula:
			%\texttt{cast(u64, iB.addr) + cast(u64, (isimm11 << 2))}.
			\texttt{iB.addr + cast(u64, (isimm11 << 2))}.
		...
		\item Instruction List:
			...
			\item \texttt{retxz dA}
				...
				\item This instruction returns from supervisor mode to user
					mode if \texttt{dA} is zero.
				\item This instruction causes an exception if the processor
					is in user mode.
				%--------
				\end{itemize}
			\item \texttt{retxnz dA}
				\begin{itemize}
				%--------
				\item This instruction returns from supervisor mode to user
	...
	\subsection{Group 12 Instructions}
		...
		\item Instruction List:
			...
			\item \texttt{addi.s dA, ipc.addr, simm20}
				...
				%--------
				\end{itemize}
			%\item \texttt{shlcpyi.s dA, simm20}
			%	\begin{itemize}
			%	%--------
			%	\item This instruction copies the value of
			%		\texttt{cast(dA.type,
			%			(cast(s64, dA.scalar\_data) << 20)
			%			| cast(s64, simm20))}
			%		to \texttt{dA.scalar\_data}.
			%	%--------
			%	\end{itemize}
			\item \textit{Reserved for future expansion.}
			\item \textit{Reserved for future expansion.}
